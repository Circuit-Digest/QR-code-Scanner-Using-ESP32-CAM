/*
  ESP32-CAM Based QR Code Reader

  Features:
  - Connects to WiFi and establishes a secure HTTPS connection with a server.
  - Captures an image using the ESP32-CAM when a button is pressed.
  - Sends the captured image to a remote server via an HTTP POST request.
  - Receives and processes the server's response, which includes QR code data and an image link.
  - Displays messages and QR code data on an OLED display.

  Components:
  - ESP32-CAM: Handles image capture and network communication.
  - SSD1306 OLED Display: Shows status messages and QR code data.
  - External I2C pins: Defined for the OLED display as ESP32-CAM doesn't have dedicated I2C pins.
  - Flashlight: Used to illuminate the scene for better image capture.
  - Trigger Button: Initiates image capture and upload process.
  ...
  - Adafruit_GFX.h and Adafruit_SSD1306.h: For OLED display functionality.
